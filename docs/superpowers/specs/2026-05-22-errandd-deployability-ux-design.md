# Errandd — Deployability, Git-Backed Jobs & UX Rewrite
...
One combined iteration on Errandd covering six features:
...
- **Config:** `src/config.ts` reads `.claude/errandd/settings.json`, `parseSettings()` builds a `Settings` object cached in-process. A few fields already read `process.env` inline (`TELEGRAM_TOKEN`, `DISCORD_TOKEN`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`).
- **Jobs:** `src/jobs.ts` loads `*.md` job files from `getJobsDir()` (default `.claude/errandd/jobs`) and per-agent `agents/<name>/jobs/`. `src/cron.ts` matches schedules.
...
## 1.1 Config + `ERRANDD_*` environment overrides
...
- Exports a declarative `ENV_OVERRIDES` table. Each entry:
  ...
  interface EnvOverride {
    env: string;            // e.g. "ERRANDD_WEB_PORT"
...
`ERRANDD_MODEL`, `ERRANDD_API`, `ERRANDD_TIMEZONE`, `ERRANDD_WEB_ENABLED`, `ERRANDD_WEB_HOST`, `ERRANDD_WEB_PORT`, `ERRANDD_API_TOKEN`, `ERRANDD_HEARTBEAT_ENABLED`, `ERRANDD_HEARTBEAT_INTERVAL`, `ERRANDD_TELEGRAM_TOKEN` (alias `TELEGRAM_TOKEN`), `ERRANDD_DISCORD_TOKEN` (alias `DISCORD_TOKEN`), `ERRANDD_SLACK_BOT_TOKEN` (alias `SLACK_BOT_TOKEN`), `ERRANDD_SLACK_APP_TOKEN` (alias `SLACK_APP_TOKEN`), `ERRANDD_SECURITY_LEVEL`, `ERRANDD_JOBSREPO_URL`, `ERRANDD_JOBSREPO_BRANCH`, `ERRANDD_JOBSREPO_INTERVAL`.
...
**Docs:** new `.env.example` listing every `ERRANDD_*` var with a short comment; README section "Configuration & environment overrides".

## 1.2 Dockerfile

**New files: `./Dockerfile`, `./.dockerignore`.**

Modeled on `teamclara/infrastructure/images/errandd/Dockerfile`:
...
- Default env: `ERRANDD_WEB_ENABLED=true`, `ERRANDD_WEB_HOST=0.0.0.0`, `ERRANDD_WEB_PORT=4632`.
...
**`getJobsDir()` change:** when `jobsRepo.url` is set, return the clone dir (`.claude/errandd/jobs-repo`). Precedence: explicit `jobsDir` setting > jobs-repo clone dir (when `jobsRepo` configured) > default `.claude/errandd/jobs`. The repo *is* the jobs dir — UI-created `quick-*` jobs write into it too.

**New file: `src/jobsRepo.ts`** — all functions shell out to `git` via `Bun.spawn`/`execFile`-style calls:

- `getJobsRepoDir(): string` → `.claude/errandd/jobs-repo`.
...
- `syncJobsRepo(): Promise<SyncResult>` — `git add -A`; if nothing staged, return `{ committed: false }`; else `git commit -m "errandd: sync jobs (<ISO timestamp>)"` then `git push origin <branch>`. Returns `{ committed, pushed, message, error? }`. Uses ambient git credentials (SSH key / `gh auth setup-git`); Errandd does not manage credentials.
...
**New file: `src/ui/services/session-meta.ts`** — manages `.claude/errandd/session-meta.json`:
...
1. Config + `ERRANDD_*` env overrides
