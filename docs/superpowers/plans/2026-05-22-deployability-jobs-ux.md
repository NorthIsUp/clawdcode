# Errandd Deployability, Git-Backed Jobs & UX Rewrite — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make Errandd deployable (env-overridable config + Dockerfile), back its jobs with a periodically-pulled git repo plus a UI editor and git-sync, and rewrite the web UI as a full-screen responsive 4-section app.
...
**Spec:** `docs/superpowers/specs/2026-05-22-errandd-deployability-ux-design.md`
...
## Task 1: `ERRANDD_*` environment overrides
...
git commit -m "feat(config): ERRANDD_* environment variable overrides"
```
...
```html
  <div class="app">
    <nav class="rail" id="rail" aria-label="Sections">
      <div class="rail-brand" title="Errandd">🦞</div>
...
ENV ERRANDD_WEB_ENABLED=true \
    ERRANDD_WEB_HOST=0.0.0.0 \
    ERRANDD_WEB_PORT=4632
...
List every `ERRANDD_*` var from `ENV_OVERRIDES` with a one-line comment each, e.g.:
```
# Model + API
ERRANDD_MODEL=
ERRANDD_API=
# Web dashboard
ERRANDD_WEB_ENABLED=true
ERRANDD_WEB_HOST=0.0.0.0
ERRANDD_WEB_PORT=4632
# Jobs git repo (empty url disables)
ERRANDD_JOBSREPO_URL=
ERRANDD_JOBSREPO_BRANCH=main
ERRANDD_JOBSREPO_INTERVAL=300
# Channels (ERRANDD_* names; bare TELEGRAM_TOKEN etc. also accepted)
ERRANDD_TELEGRAM_TOKEN=
ERRANDD_DISCORD_TOKEN=
ERRANDD_SLACK_BOT_TOKEN=
ERRANDD_SLACK_APP_TOKEN=
```
(Include all the others from the table — model/api/fallback/timezone/api-token/heartbeat/security/stt.)

- [ ] **Step 5: README docs**

Add two README sections: "Configuration & environment overrides" (settings.json is the source of truth; any field can be overridden by its `ERRANDD_*` var; nested arrays/objects are file-only; jobs repo config) and "Run with Docker" (`docker build -t errandd .`, `docker run -p 4632:4632 -v $PWD/.claude:/app/.claude --env-file .env errandd`; Claude auth via the mounted `.claude` volume or `CLAUDE_CODE_OAUTH_TOKEN`).

- [ ] **Step 6: Build the Docker image to verify**

Run: `docker build -t errandd:test .`
