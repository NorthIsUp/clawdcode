...
export function buildSecurityArgs(security: SecurityConfig): string[] {
  ...
  // Output style — merged in as inline extra settings so it rides along on
  // every spawn site (all of which spread these args).
  args.push(...outputStyleArgs(getSettings().outputStyle));
