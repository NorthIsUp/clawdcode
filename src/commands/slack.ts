...
const SLACK_OUTBOX_DIR = join(process.cwd(), ".claude", "errandd", "outbox", "slack");
...
async function downloadSlackFile(
  ...
  const dir = join(process.cwd(), ".claude", "errandd", "inbox", "slack");
...
async function handleMessage(event: SlackMessage): Promise<void> {
  ...
  try {
    ...
    // Plugin wizard intercept — /plugin and /errandd:plugin are handled here, not by Claude
    ...
    if (result.exitCode !== 0) {
      ...
      for (const read of channelReads) {
        ...
        try {
          ...
          const historyPath = join(process.cwd(), ".claude", "errandd", "inbox", "slack", `channel-${read.channelId}-${Date.now()}.txt`);
          await mkdir(join(process.cwd(), ".claude", "errandd", "inbox", "slack"), { recursive: true });
...
export async function slack(): Promise<void> {
  ...
  if (!config.botToken) {
    console.error("Slack bot token not configured. Set slack.botToken in .claude/errandd/settings.json");
    process.exit(1);
  }
  if (!config.appToken) {
    console.error("Slack app token not configured. Set slack.appToken in .claude/errandd/settings.json");
    process.exit(1);
  }
