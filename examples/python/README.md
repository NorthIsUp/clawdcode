...
Standalone Python modules that extend Errandd agents with crash-recovery
...
Fernet-encrypted (AES-128-CBC + HMAC-SHA256) secrets vault for Errandd
...
The vault key is stored at `~/.errandd/vault/.vault_key` (chmod 600).
Override with the `ERRANDD_VAULT_KEY` environment variable.
