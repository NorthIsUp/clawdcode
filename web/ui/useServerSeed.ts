 ...
 *
 * The common pattern: an async source returns `data`, and the panel needs a
 ...
 * but not on every render — otherwise the user's edits get clobbered.
 *
 * Returns the draft + setter, so callers behave like a normal `useState`
 * thereafter. The `mapper` runs on each new server payload and produces the
 * initial draft shape.
 *
