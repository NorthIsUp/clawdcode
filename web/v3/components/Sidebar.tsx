...
function loadCollapsed(): CollapseMap {
  try {
    ...
    if (raw) return JSON.parse(raw) as CollapseMap;
  } catch {
  ...
  return (
    <>
      <div className="flex items-center gap-2 border-b border-base-300 px-3 py-3">
        ...
        <span className="font-semibold tracking-tight">ClawdCode</span>
        <span
          className={cn(
        ...
        >
          <span
            className={cn(
              ...
              connected ? "animate-pulse bg-success" : "bg-base-content/30",
            )}
          />
          v3
        </span>
      ...
      <div className="flex-1 overflow-y-auto">
        {error ? (
          <p className="px-3 py-4 text-xs text-error">
            Failed to load hook queue: {error.message}
          </p>
        ) : loading ? (
