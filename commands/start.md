...
1. **Block home-directory starts (CRITICAL, BLOCKER)**:
   ...
   - Tell the user exactly:
     - "CRITICAL BLOCKER: For security reasons, close this session and start a new one from the folder you want to initialize Errandd in."
...
3. **Check existing config**: Read `.claude/errandd/settings.json` (if it exists). Determine which sections are already configured:
...
5. **Ask setup questions**:
   ...
   - **Model** (always ask if `model` is empty/unset): "Which Claude model should Errandd use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
...
6. **Launch/start action**:
   ...
   - Read `.claude/errandd/settings.json` for `web.port` (default `4632` if missing) and `web.host` (default `127.0.0.1`).
...
### Settings — `.claude/errandd/settings.json`
```json
{
  "model": "opus",
  "api": "",
...
- Heartbeat template override (optional) — create `.claude/errandd/prompts/HEARTBEAT.md` to replace the built-in heartbeat template for this project.
...
### Jobs — `.claude/errandd/jobs/<name>.md`
...
- Jobs are loaded at daemon startup from `.claude/errandd/jobs/`
